...
## Current Sprint
None — ready for next implementation run. PR #16 (Cellar text search) merged, closing the prior Current Sprint item.
...
- **What:** `Dashboard.tsx` (line 55) and `useScannerLogic.ts` (line 58) both correctly handle `profile?.use_shared_key` — the setup banner and the "API key required" guard both already account for shared-key users. The remaining gap: `Settings.tsx` (117 lines, confirmed June 11) still has zero references to `use_shared_key`, `useAuth`, or `profile` — the API key input section renders identically for everyone, regardless of whether an admin has granted them shared-key access.
- **Why now:** Now a small, self-contained UI change — both the edge function guard and the Dashboard banner logic are already correct, making this genuinely S-effort. It's the single largest remaining onboarding dead-end for non-technical users who've been granted shared-key access but still see a confusing "enter your API key" form telling them the scanner won't work without one.
- **Effort estimate:** S
- **Actual effort:** —
...
- **Settings.tsx shared-key UX is now the bellwether item.** It has been correctly scoped as S-effort and shovel-ready for 2 consecutive cycles (June 10, 11) without being picked up. If it's still open at the next assessment, that's worth flagging to the human reviewer as a process question (is the implementation routine running? is something blocking PR creation?) rather than continuing to re-describe the same gap a third time.
