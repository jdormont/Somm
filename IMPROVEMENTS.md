...
_Assessment based on: fresh code read of `src/pages/ScanDetail.tsx` (stale chosenWine init confirmed), `src/pages/Cellar.tsx` (no text search confirmed; raw supabase calls without React Query confirmed), `CLAUDE.md` (absent — 404 confirmed), `src/components/RecommendationCard.tsx`, `src/pages/Settings.tsx`; git log (last 30 commits), all PRs (none open), open issues (none). No commits since June 3 assessment._
...
- **What:** `ScanDetail.tsx` initializes `chosenWine` state with `useState(session?.chosen_wine_name ?? null)`, but `session` is `undefined` on the first render (React Query loads async). Confirmed June 4: no `useEffect` syncs the state after the session resolves, so the "I Chose This" toggle always renders unchosen when a user navigates directly to a scan URL — even if they previously made a choice. The fix is a one-line `useEffect(() => { setChosenWine(session?.chosen_wine_name ?? null); }, [session?.chosen_wine_name])`.
...
- **What:** The Somm repo still has no `CLAUDE.md`. Confirmed absent (June 4: file returns 404). Every AI-assisted development session starts cold with no knowledge of the stack, design system, admin-approval flow, API key model, or branching conventions. This has been noted in each of the past four assessments (June 1, June 2, June 3, June 4) without action.
- **Why now:** Four consecutive assessments without movement. A 30-minute task with the best effort-to-session-productivity ratio of anything in the backlog. The cost of skipping it is paid on every future session.
- **Effort estimate:** S
- **Actual effort:** —
...
- **Why now:** The Dashboard received text search in PR #5. The Cellar is the other primary browsing surface and has the same gap — parallel treatment brings UX consistency. Migrating to React Query as part of this fix closes the architectural inconsistency before more cellar features are added.
- **Effort estimate:** S
- **Actual effort:** —
...
- **Why now:** Consistently identified as medium impact across prior assessments. Good candidate for a focused sprint when mobile UX improvements are prioritized. Migrate Cellar to React Query first (Tier 1 item above) — the React Query cache is a natural foundation for offline hydration.
- **Effort estimate:** M
- **Actual effort:** —
