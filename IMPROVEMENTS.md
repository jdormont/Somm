...
| Item | Status | Notes |
|------|--------|-------|
| Staged progress feedback during scan analysis (Tier 1) | ✅ DONE — PR #24, 2026-06-22 | `Scanner.tsx` now rotates through "Reading the wine list...", "Scoring matches against your taste profile...", "Researching your top picks...", "Finalizing recommendations..." on a `setInterval` timed to the actual pipeline phases (0s/5s/12s/22s), instead of a single static label. Purely client-side; no backend/edge-function changes. |
...
- **Why now:** Carried from prior assessments; remains the most-requested type of feature for power users.
- **Effort estimate:** S
- **Actual effort:** —
