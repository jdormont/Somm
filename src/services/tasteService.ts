...
export const tasteService = {
  ...
  async analyzeAnchorWine(payload: AnchorAnalysisRequest): Promise<AnchorAnalysisResult> {
    ...
    if (error) {
       ...
       try {
         // @ts-expect-error -- Supabase error context type is not fully exposed but exists at runtime
         if (error.context && typeof error.context.json === 'function') {
            // @ts-expect-error -- accessing runtime context.json() not in Supabase type definitions
